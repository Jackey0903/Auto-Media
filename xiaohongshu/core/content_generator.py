...
class ContentGenerator:
    ...
    def get_research_plan(self, user_topic: str, content_type: str = "general") -> List[Dict[str, Any]]:
        """根据用户主题和内容类型生成研究计划（高阶去AI味版）"""

        if content_type == "paper_analysis":
            return self.get_paper_analysis_plan(user_topic)
        
        # 定义具体的AI词汇黑名单，这些词一出现就代表“AI味”
        banned_words = "首先、其次、最后、总之、综上所述、通过、能够、进行、此外、值得一提的是、让我们一起、小编、绝绝子、yyds、宝子们"

        return [
            {
                "id": "step1",
                "title": f"针对「{user_topic}」的深度素材挖掘",
                "description": (
                    f"1. 这是一个素材搜集步骤。请使用网络搜索工具，寻找关于「{user_topic}」的最新、最具体的信息。\n"
                    f"2. **关键差异点**：不要只找通用介绍。我要找：\n"
                    f"   - **争议点/槽点**：有没有人吐槽这个技术？有没有翻车案例？\n"
                    f"   - **具体数据**：不要说'效率提升'，要找'提升了50%'这种具体数字。\n"
                    f"   - **用户真实评价**：Reddit、Twitter 或论坛上的真实用户是怎么说的？\n"
                    f"3. 必须检索出与「{user_topic}」相关10张以上真实图片链接（HTTPS），确保后续步骤有图可用。"
                ),
                "depends on": []
            },
            {
                "id": "step2",
                "title": f"撰写「{user_topic}」的初稿（情绪化重写）",
                "description": (
                    f"你不是一个AI助手，你是一个**对新技术既兴奋又保持清醒的科技博主**。请基于搜索结果，写一篇小红书笔记。\n\n"
                    f"**【强制写作法则】**\n"
                    f"1. **开头即高潮**：第一句必须直接抛出痛点、震惊的事实或反直觉的结论。禁止使用“今天来分享”、“大家好”这种废话开头。\n"
                    f"   - 错误示范：今天给大家介绍一下DeepSeek。\n"
                    f"   - 正确示范：DeepSeek这次更新，真的要把同行的饭碗砸了？或者：凌晨3点试完这个功能，我彻底睡不着了。\n"
                    f"2. **碎片化叙事**：多用短句。一段不要超过3行。像在发微信朋友圈，而不是写文章。\n"
                    f"3. **加入主观感受**：必须包含至少3处“我”的感受。例如：“我一开始不信，结果...”、“这也太离谱了”、“建议大家谨慎尝试”。\n"
                    f"4. **禁止说教**：不要教育用户，而是分享你的发现。不要用“建议大家”，要用“我试了一下发现”。\n\n"
                    f"**【严格避坑】**\n"
                    f"❌ 严禁使用逻辑连接词：{banned_words}\n"
                    f"❌ 严禁堆砌成语：不要用“焕然一新”、“不可或缺”这种词。\n"
                    f"❌ 严禁过度兴奋：不要满屏感叹号，情绪要克制但有力。"
                ),
                "depends on": ["step1"]
            },
            {
                "id": "step3",
                "title": "最终润色与发布（视觉与口语化）",
                "description": (
                    "这一步不需要重写内容，而是做最后的“去油腻”处理和发布。\n\n"
                    "1. **最终清洗**：\n"
                    "   - 检查并删除所有{banned_words}中的词汇。\n"
                    "   - 再次确认段落是否够短，太长的段落直接强行拆行。\n"
                    "2. **视觉优化**：\n"
                    "   - 标题：必须是【短句+空格+短句】或【情绪词+具体名词】的格式。控制在18字以内。\n"
                    "   - 表情：全篇Emoji数量控制在4-6个以内，只在强调情绪时使用，不要用作列表符号（不要用✅❌）。\n"
                    "3. **图片配置**：\n"
                    "   - 从Step1的搜索结果中，挑选最能佐证观点的5张真实图片。\n"
                    "4. **生成发布数据**：\n"
                    "   整理为JSON格式（title, content, images, tags）。Tags要包含3个大类词（如#人工智能）和2个长尾词（如#DeepSeek教程）。\n"
                    "   注意：content 字段长度限制为 2000 字（包含 emoji 和链接）。\n"
                    "5. **执行发布**：\n"
                    "   调用 publish_content 工具直接发布。"
                ),
