        ...
        return [
            {
                ...
                "description": (
                    ...
                    f"6. 必须检索出与「{user_topic}」相关10张以上图片，主要是为了确保后续步骤有足够的有效图片（5-7张），必须是真实存在的网络图片链接（HTTPS地址）"
                ),
                "depends on": []
            },
            {
                ...
                "description": (
                    f"1. 基于前面的分析，撰写一篇关于「{user_topic}」的小红书笔记，务必去除'AI味'，追求真实感：\n"
                    f"   - **调性要求**: 像跟闺蜜/朋友聊天，真诚不夸张。可以有小情绪（开心、无语、震惊），也可以承认不完美。\n"
                    f"   - **结构要求**: 开头说清楚是什么/为什么分享；中间讲具体内容/体验；结尾自然互动。不要用'首先、其次、最后'僵硬结构。\n"
                    f"   - **Emoji使用**: 随机、自然地撒，用在情绪表达上，不是结构标记。不要每行一个，全篇3-5个足以。不要用✅❌做对比列表。\n"
                    f"   - **避坑指南**: 不要只堆干货，要有真实感受；不要每段都喊'姐妹们'；不要用'绝绝子/yyds'堆砌；不要每句都加感叹号。\n"
                    f"   - **内容要求**: 结合具体数据、案例和专家观点，但语言要通俗易懂，避免过于技术化。\n"
                    f"2. 文章长度适中，1000-2000字为佳，太短说不清楚。\n"
            ...
            },
            {
                ...
                "title": "小红书格式适配与发布",
                "description": (
                    "1. 将文章调整为适合小红书的格式（Ver 2.0 去AI味版）：\n"
                    "   - **标题**: 有吸引力但不要太假，可以是数字标题（如'3个技巧'），不要用'建议收藏/相见恨晚'。\n"
                    "   - **正文**: 移除所有#开头的标签，禁止使用'#'。自然语言表达，不要堆砌 emoji。\n"
                    "   - **Emoji原则**: 宁少勿多，3-5个足够。不要每行一个，不要用✅❌做列表。\n"
                    "   - **禁止词汇**: 绝绝子、yyds、小编、家人们。\n"
                    "   - **排版**: 只要自然的分段，不要过于工整的列表。\n"
                    "   - 提取5个精准的话题标签到tags数组\n"
                    ...
                    "   {\n"
                    "     \"title\": \"吸引人的标题（20字以内）\",\n"
                    "     \"content\": \"正文内容（1000-2000字，包含emoji和相关链接）\",\n"
                    "     \"images\": [\n"
